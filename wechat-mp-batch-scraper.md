...
> 🙏 **致谢 · 站在巨人的肩膀上**
>
> 本方案**不是从零原创**，而是基于 **mcncarl** 的开源 Skill 「**wechat-mp-batch-exporter**」（仓库 `yichen-skills`）改良而来——核心的抓取 / 导出思路都来自原作者，特此致谢。
>
> 👉 **原作者原始 Skill（建议先看）**：https://github.com/mcncarl/yichen-skills/tree/main/wechat-mp-batch-exporter
>
> 我在原作基础上做的，只是**在自己实际使用中把流程打磨得更顺手**：补上了「文章链接获取的『人在回路』方案、反爬会话预热、终端密钥防脱敏」等一手实操踩坑，整理成这份中文版，也分享给同样有需要的人。感谢原作者的开源工作。

> 适用场景：需要把微信公众号文章正文批量扒下来，回填到表格某一列或做归档。
> 本文记录从「直接抓 → 踩坑 → 跑通」的完整过程，以及最终方案。**第一版实测发现「获取文章链接列表」才是真正卡点，本版重点补上「人在回路」的解法。** 在 mcncarl 开源 Skill 基础上，由 AI（Workbuddy）协助整理实操版，供参考。
